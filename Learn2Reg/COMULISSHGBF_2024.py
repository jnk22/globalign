...
def register(ref_image, flo_image, k=8, grid_angles=100, refinement_angles=32):
    ...
    Q_B = k
    print(
        f"Using k={Q_A} and k={Q_B} quantization levels for ref. and flo. images, respectively"
    )

    quantized_ref_image = nd2cat.image2cat_kmeans(ref_image, Q_A)
    quantized_flo_image = nd2cat.image2cat_kmeans(flo_image, Q_B)
    print(
        f"Quantized ref. image shape: {ref_image.shape}, Quantized flo. image shape: {flo_image.shape}",
        flush=True,
    )
    ...
    param = globalign.align_rigid_and_refine(
        ...
        grid_angles,
        180.0,
        refinement_param=refinement_param,
        overlap=overlap,
...
def align(ref, flo):
    ...
    warpedpoints = globalign.warp_points_rigid(
        gridpoints, param[0]
    )  # Warp the points (ref->flo)
    displace = (
        warpedpoints.reshape(grid.shape) - grid
    )  # Subtract the identity grid to make displacement only
    displace = np.swapaxes(displace, 0, 1)  # Empirically found to be needed

    displace = np.expand_dims(
        displace, axis=2
    )  # Reshape to [h,w,1,2], i.e., [X,Y,1,[x,y]]
...
if __name__ == "__main__":
    ...
    os.mkdir(
        "output"
    )  # Intentionally fail if the directory exists, to not risk overwriting
    ...
    with open(os.path.join(BASE_PATH, "COMULISSHGBF_dataset.json")) as f:
        ...
        for x in d["registration_val"]:
            align(
                os.path.join(BASE_PATH, x["fixed"]),
                os.path.join(BASE_PATH, x["moving"]),
            )
